...
- **Archive skew repair**: Before Certbot runs, **`deploy.sh`** (via a root Docker one-shot — host users often cannot list root-owned **`archive/`**) removes orphan **`privkeyN.pem`** versions above the live symlink, and on **`FileExistsError`** deletes the conflicting version (retargeting live backward first if needed) then retries once.
