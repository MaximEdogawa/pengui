  ...
  return (
    <AppLink href={href} className={linkClass}>
      ...
      <div className="flex flex-col items-end flex-shrink-0 min-w-0 max-w-[50%] sm:max-w-none">
        <p
          className={`${t.text} text-[15px] sm:text-sm font-semibold tabular-nums text-right truncate w-full`}
          title={`${formatBalance(asset.balance, asset.type)} ${isLpToken ? lpTicker : asset.ticker}`}
        >
          {formatBalance(asset.balance, asset.type, maxDecimals)}{" "}
          {isLpToken ? lpTicker : asset.ticker}
        </p>
        <p
          className={`${t.textSecondary} text-[13px] sm:text-xs tabular-nums text-right truncate w-full`}
          title={formatUsd(asset.balanceUsd)}
        >
