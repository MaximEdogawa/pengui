...
export function formatOfferLine(offer: DexieOffer): string {
  const offered = offer.offered?.map((a) => `${a.code}:${a.amount}`).join(" ") ?? "";
  const requested = offer.requested?.map((a) => `${a.code}:${a.amount}`).join(" ") ?? "";
  const id = offer.id || "-";
  const price = typeof offer.price === "number" ? offer.price.toFixed(4) : "-";
