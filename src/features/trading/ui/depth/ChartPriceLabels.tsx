...
export function ChartPriceLabels({ priceLabels, chartHeight, chartWidth, priceRange }: ChartPriceLabelsProps) {  
  ...
  return (
    <>
      {middleLabels.map((label, idx) => {
        ...
        const nextLabel = idx < middleLabels.length - 1 ? middleLabels[idx + 1] : priceLabels[priceLabels.length - 1] // Last label if last middle
      ...
      <text
        ...
        fontWeight="500"
      >
        Price Range: {formatPriceForDisplay(priceRange.min)} - {formatPriceForDisplay(priceRange.max)}
      </text>
    </>
  )
}
