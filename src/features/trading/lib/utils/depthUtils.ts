...
export function aggregateDepthByPriceLevel(
  ...
  const allBidLevels = Array.from(bidMap.values()).sort((a, b) => b.price - a.price); // Descending (best bid first)

  const allAskLevels = Array.from(askMap.values()).sort((a, b) => a.price - b.price); // Ascending (best ask first)
