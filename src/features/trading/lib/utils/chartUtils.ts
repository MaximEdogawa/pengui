...
function isValidTrade(trade: unknown): trade is DexieHistoricalTrade {
  ...
  const timestampNum =
    typeof timestamp === "number"
      ...
      : typeof timestamp === "string"
        ...
        : NaN;
  const tradeTimestampNum =
    typeof tradeTimestamp === "number"
      ? tradeTimestamp
      : typeof tradeTimestamp === "string"
        ? parseFloat(tradeTimestamp as string)
        : NaN;

  const hasTimestamp =
    (!isNaN(timestampNum) && timestampNum > 0) ||
    (!isNaN(tradeTimestampNum) && tradeTimestampNum > 0);
  ...
  const volumeNum =
    typeof volume === "number" ? volume : typeof volume === "string" ? parseFloat(volume) : NaN;
  const baseVolumeNum =
    typeof baseVolume === "number"
      ? baseVolume
      : typeof baseVolume === "string"
        ? parseFloat(baseVolume as string)
        : NaN;
  const targetVolumeNum =
    typeof targetVolume === "number"
      ? targetVolume
      : typeof targetVolume === "string"
        ? parseFloat(targetVolume as string)
        : NaN;

  const hasVolume =
    (!isNaN(volumeNum) && volumeNum > 0) ||
    (!isNaN(baseVolumeNum) && baseVolumeNum > 0) ||
    (!isNaN(targetVolumeNum) && targetVolumeNum > 0);
    ...
    validationChecks: {
      hasPrice:
        ...
        (typeof sample.price === "number" ||
          (typeof sample.price === "string" && !isNaN(parseFloat(sample.price)))),
      hasTimestamp:
        ("timestamp" in sample &&
          (typeof sample.timestamp === "number" ||
            (typeof sample.timestamp === "string" &&
              !isNaN(parseFloat(sample.timestamp as string))))) ||
        ("trade_timestamp" in sample &&
          (typeof sample.trade_timestamp === "number" ||
            (typeof sample.trade_timestamp === "string" &&
              !isNaN(parseFloat(sample.trade_timestamp as string))))),
      hasVolume:
        ("volume" in sample &&
          (typeof sample.volume === "number" ||
            (typeof sample.volume === "string" && !isNaN(parseFloat(sample.volume as string))))) ||
        ("base_volume" in sample &&
          (typeof sample.base_volume === "number" ||
            (typeof sample.base_volume === "string" &&
              !isNaN(parseFloat(sample.base_volume as string))))) ||
        ("target_volume" in sample &&
          (typeof sample.target_volume === "number" ||
            (typeof sample.target_volume === "string" &&
              !isNaN(parseFloat(sample.target_volume as string))))),
    },
  });
}
...
export function aggregateTradesToOHLC(
  ...
  logger.info("Trade aggregation summary", {
    ...
    timeframe,
    tradesPerCandle:
      validTrades.length > 0 ? (validTrades.length / sortedCandles.length).toFixed(2) : 0,
...
export function transformOrderBookForChart(
  ...
  if (buyOrders && sellOrders && calculatePriceFn) {
    ...
    const nativeTicker =
      filters?.buyAsset?.find((a) => a === "XCH" || a === "TXCH") ||
