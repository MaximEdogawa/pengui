...
import {
  transformOrderBookForChart,
  generateSyntheticOHLCFromOrderBook,
  invertOHLCData,
...
export function usePriceChart({ config, isUserScrolling = false }: UsePriceChartOptions) {
  ...
  const isUsingSyntheticData =
    ohlcData.length === 0 && !hasCachedData && !isPriceDataSuccess && syntheticOHLC.length > 0;
