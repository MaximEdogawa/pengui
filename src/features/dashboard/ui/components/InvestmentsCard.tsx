  ...
  switch (status) {
    ...
    case "active":
      return isDark
        ...
        : "text-emerald-700 bg-emerald-500/15 border-emerald-600/20";
    case "available":
      return isDark
        ? "text-blue-400 bg-blue-500/10 border-blue-400/20"
        : "text-blue-700 bg-blue-500/15 border-blue-600/20";
    case "settled":
      return isDark
        ? "text-slate-400 bg-slate-500/10 border-slate-400/20"
        : "text-slate-600 bg-slate-500/15 border-slate-600/20";
    default:
      return isDark
        ? "text-amber-400 bg-amber-500/10 border-amber-400/20"
        : "text-amber-700 bg-amber-500/15 border-amber-600/20";
  }
}
    ...
    >
      ...
      {isEmpty ? (
        ...
        <>
          {/* Table header */}
          <div
            className={`grid grid-cols-12 gap-2 px-2.5 py-1.5 text-[10px] font-medium uppercase tracking-wide ${t.textTertiary}`}
          >
          ...
          <div className="space-y-1">
            {rows.map((row) => (
              ...
              >
                ...
                <span
                  className={`col-span-2 text-right ${t.text} text-[11px] font-semibold tabular-nums`}
                >
                  {row.value}
                </span>
                ...
                <span
                  className={`col-span-2 text-right ${t.textTertiary} text-[10px] tabular-nums`}
                >
                  {row.date}
                </span>
              </AppLink>
