...
function TxIcon({ type, isDark }: { type: StoredTransaction['type']; isDark: boolean }) {
  ...
  switch (type) {
    case 'swap':
      return <ArrowRightLeft size={size} strokeWidth={stroke} className={isDark ? 'text-blue-400' : 'text-blue-600'} />
    case 'trade':
      return <Repeat size={size} strokeWidth={stroke} className={isDark ? 'text-violet-400' : 'text-violet-600'} />
    default:
      return incoming
  ...
  return (
    ...
    >
      ...
      <div className="mb-2">
        ...
        <p className={`${t.textTertiary} text-[10px] font-medium`}>
          Profit / Loss
        </p>
      </div>
              ...
              <div className="flex items-center gap-2.5">
                  ...
                  className={`p-1.5 rounded-lg backdrop-blur-sm flex items-center gap-1.5 ${
                    tx.type === 'receive'
                      ...
                      : tx.type === 'swap' || tx.type === 'trade'
                        ...
                        : isDark ? 'bg-rose-500/10' : 'bg-rose-500/20'
                  }`}
                >
                ...
                <div>
                  <p className={`${t.text} font-medium text-[11px]`}>
                    {txTypeLabel(tx.type)}{tx.amountAsset ? ` · ${tx.amountAsset}` : ''}
                  </p>
