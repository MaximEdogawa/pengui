...
export function handleErrorCode4001(
  ...
  if (
    ...
    return {
      success: false,
      error:
        "Wallet rejected the request. This may be due to invalid parameters, network mismatch, or the wallet being unable to process the request. Please check the wallet and try again.",
    };
  }
  ...
  return {
    ...
    error:
      errorMessage ||
      "The wallet could not process the request. Please check the wallet and try again.",
  };
}
...
export function handleWalletRequestError(
  ...
  if (isRelayError(errorMessage)) {
    return {
      success: false,
      error: "Relay communication error. Please try again.",
    };
  }
  ...
  if (errorCode !== undefined) {
    return {
      success: false,
      error: errorMessage || `Wallet error (code ${errorCode})`,
    };
  }
