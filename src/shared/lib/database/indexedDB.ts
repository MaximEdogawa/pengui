...
export class PenguiDB extends Dexie {
  ...
  constructor() {
    ...
    this.version(2)
      .stores({
        offers:
          "++id, id, tradeId, status, createdAt, lastModified, walletAddress, syncedAt, network",
      })
      .upgrade(async (tx) => {
    ...
    this.version(3)
      .stores({
        offers:
          "++id, id, tradeId, status, createdAt, lastModified, walletAddress, syncedAt, network, takenBy",
      })
      .upgrade(async (tx) => {
