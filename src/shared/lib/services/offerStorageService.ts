...
export class OfferStorageService {
  ...
  async saveOffer(
    ...
    try {
      ...
      if (existingOffer) {
        logger.info("⚠️ Offer already exists, updating instead of creating:", {
          offerId: offer.id,
        });
   ...
   */
  private async removeDuplicateOffer(offerId: string, duplicate: StoredOffer): Promise<void> {
  ...
  async getOffersByStatus(
    ...
    try {
      ...
      logger.info("✅ Retrieved offers by status from IndexedDB:", {
        status,
        count: offers.length,
      });
  ...
  async markOfferAsTaken(offerId: string, walletAddress: string): Promise<void> {
    try {
      ...
      logger.info("✅ Offer marked as taken in IndexedDB:", {
        offerId,
        walletAddress,
        updatedCount,
      });
      ...
      for (const offer of allOffers) {
        ...
        const isMyOffer =
          (offer.creatorAddress &&
            offer.creatorAddress.toLowerCase() === walletAddress.toLowerCase()) ||
          (offer.walletAddress &&
            offer.walletAddress.toLowerCase() === walletAddress.toLowerCase()) ||
