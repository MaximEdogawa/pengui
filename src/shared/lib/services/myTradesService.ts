import { db, type StoredOffer, ensureDatabaseReady, isDatabaseReady, withTimeout } from '@/shared/lib/database/indexedDB'
...
export class MyTradesService {
  ...
  async getMyTrades(
    ...
    try {
      ...
      if (filters?.dateFrom) {
        filteredTrades = filteredTrades.filter((trade) => trade.timestamp >= filters.dateFrom!.getTime())
      }

      if (filters?.dateTo) {
        filteredTrades = filteredTrades.filter((trade) => trade.timestamp <= filters.dateTo!.getTime())
      }
      ...
      return filteredTrades.map((trade) => {
        const entryPrice = this.getEntryPrice(trade.ticker_id || '', walletAddress, trade.type, filteredTrades)
        if (entryPrice) {
  ...
  convertOfferToTrade(offer: StoredOffer, walletAddress: string): TradeHistoryItem | null {
    try {
      ...
      const price = requestedAsset.amount > 0 && offeredAsset.amount > 0 
   ...
   */
  calculateProfitLoss(trade: TradeHistoryItem, entryPrice: number): { absolute: number; percent: number } {
  ...
  identifyMyTrades(
    ...
    apiTrades.forEach((apiTrade) => {
      ...
      const matchingTrade = Array.from(myTradesMap.values()).find((myTrade) => {
        ...
        const timeMatch = myTrade.timestamp && apiTrade.trade_timestamp
