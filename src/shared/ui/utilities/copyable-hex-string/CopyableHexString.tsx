...
export function CopyableLongHexString({
  hexString,
  className,
  tooltipText = "Copy",
}: {
  ...
  const copyElement = (
    <p
      className={cn(
        `opacity-80 select-none hover:opacity-60 bg-accent w-fit px-2 py-0.5 text-sm rounded text-primary/80`,
        isCopied ? "opacity-60" : "cursor-pointer",
        className
      )}
