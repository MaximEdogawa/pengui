...
async function test(
  name: string,
  url: string,
  checks: { status?: number; contentType?: string; bodyJson?: (data: unknown) => boolean }
): Promise<Result> {
  try {
    const res = await fetch(url, { method: "GET" });
    const statusOk = checks.status == null || res.status === checks.status;
    const ct = res.headers.get("content-type") ?? "";
    const contentTypeOk =
      checks.contentType == null || ct.toLowerCase().includes(checks.contentType.toLowerCase());
